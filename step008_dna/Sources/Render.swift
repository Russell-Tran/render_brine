// A small GPU ray tracer for ball-and-stick molecules (from step 7a).
//
// New in step 8: a DNA duplex has ~760 atoms and ~1,100 sticks, far more than
// fits in the 4 KB that setBytes allows. So the scene now lives in two
// MTLBuffers in shared memory, which the CPU fills and the GPU reads with no
// copy, and the kernel reads them from the `device` address space instead of
// `constant` (which suits small tables read by every thread).
//
...
let raytraceKernelSource = """
    ...
    // A faint, fixed ordered dither (a 4 × 4 Bayer matrix, ±1.5/255). GIFs
    // have at most 256 colors, so smooth gradients turn into visible bands;
    // this breaks the bands up. Earlier steps used per-pixel random noise; a
    // repeating pattern hides banding as well and compresses better.
    ...
    kernel void render(device uchar4 *pixels [[buffer(0)]],
        ...
        // Dither only the background (in proportion at a molecule's edge): it's
        // there to hide banding in the gradient, and on the turning molecule it
        // would just add churn that the GIF has to store every frame.
