// A GPU ray tracer for space-filling molecules, carried over from step 9 with
// one change: this subject's axis lies along x, not y.
//
// Step 9's AMBIENT OCCLUSION. Every earlier step cast one ray per sample
...
//
// Keeping the loop from crawling — and why the camera orbits about x
// -------------------------------------------------------------------
// Step 8 spun the duplex and held the camera still. That is fine for balls on
// sticks, but occlusion computed on a moving subject would be recomputed
// against different geometry every frame and would crawl. So the duplex never
// moves: it lies along x in its own coordinates and the camera circles the x
// axis instead, with the lights rotated the same way. The picture is identical
// to turning the helix under a fixed camera.
//
...
struct Camera {
    ...
    var tanHalfFOV: Float
    /// Which way is up for this camera. Orbiting about x carries the camera
    /// straight over the subject, where a fixed world up would go degenerate.
    var upHint: SIMD3<Float> = SIMD3(0, 1, 0)

    var forward: SIMD3<Float> { simd_normalize(target - origin) }
    var right: SIMD3<Float> { simd_normalize(simd_cross(forward, upHint)) }
    ...
    /// A camera `distance` Å away, circling the x axis — the helix's own axis —
    /// by `roll` degrees, and pushed `lean` degrees off the perpendicular so the
    /// duplex is seen slightly end-on rather than dead flat.
    static func orbitAboutX(target: SIMD3<Float>, distance: Float, roll: Float,
                            lean: Float, fov: Float) -> Camera {
        let r = radians(roll), l = radians(lean)
        let offset = SIMD3<Float>(sin(l), sin(r) * cos(l), cos(r) * cos(l)) * distance
        // Perpendicular to the orbit, in the plane across the axis, so x stays
        // roughly horizontal on screen all the way round.
        let up = SIMD3<Float>(0, cos(r), -sin(r))
        return Camera(origin: target + offset, target: target,
                      tanHalfFOV: tan(radians(fov / 2)), upHint: up)
    }
...
final class MoleculeRenderer {
    ...
    func render(spheres: [GPUSphere], cylinders: [GPUCylinder], camera: Camera, into frame: MTLBuffer,
                ...
                ao: AOSettings = AOSettings(), lightRoll: Float = 0) throws -> Double {
        // Turn the world-space lights into object space by unwinding the same
        // roll the camera was given.
        let keyDir = rotateAboutX(keyLightWorld, degrees: -lightRoll)
        let fillDir = rotateAboutX(fillLightWorld, degrees: -lightRoll)
...
/// Turns a vector `degrees` about the x axis — the helix's own axis, which is
/// what the camera circles, so the lights have to be unwound the same way.
func rotateAboutX(_ v: SIMD3<Float>, degrees: Float) -> SIMD3<Float> {
    let a = radians(degrees)
    let c = cos(a), s = sin(a)
    let y: Float = v.y * c - v.z * s
    let z: Float = v.y * s + v.z * c
    return SIMD3(v.x, y, z)
}
